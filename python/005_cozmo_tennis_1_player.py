...
'''COZMO Tennis For 1 Player

Ce programme se veut un jeu de tennis se jouant seul contre COZMO.
...
#NOTE : Le robot tape sur les positions 3,4,5,6,7 
import time
import random
import asyncio
import cozmo
...
def cozmo_play():
    ...
    global ball_direction
    global ball_position
    global position_to_tap
    global ball_speed
    global ready
    global service
    global scj1
    global scj2
    if (ball_direction == 1 and ball_position == position_to_tap):
        robot.play_anim_trigger(cozmo.anim.Triggers.BouncerIdeaToPlay).wait_for_completed()
        position_to_tap = random.randint(3, 7)
        # Fond de court on ralenti la balle
        if (ball_position == 6):
            ball_direction = -1
            if (ball_speed < 0.5):
...
def cozmo_program(_robot: cozmo.robot.Robot):
    ...
    robot.say_text("La partie rouge de ton coté").wait_for_completed()    
    robot.say_text("La partie bleu de mon coté").wait_for_completed()    
    i=0
    while (i < 3):
        draw_field_intro()
        i += 1
    ...
    while (scj1<5 and scj2<5):
        ...
        if (service == 1):
            ...
            ball_position = 6	
            # Le robot attend une seconde puis sert
            time.sleep(1)	
            robot.play_anim_trigger(cozmo.anim.Triggers.BouncerIdeaToPlay).wait_for_completed()
            position_to_tap = random.randint(3, 7)
            ready = True
        ...
        while (ready is False):
            ...
            if (target.obj.cube_id is LightCube1Id and service == 1):
                ready = True
        # Echange
        ready = False
        ...
        while (ready is False):
            draw_field()
            draw_ball()
            time.sleep(ball_speed/2)
            if (cozmo_play() is False):
                time.sleep(ball_speed/2)
            ball_position += ball_direction
            if (ball_position < 0):
                scj2 += 1
        ...
        if (service == 1):
            ...
            robot.play_anim_trigger(cozmo.anim.Triggers.CodeLabNo).wait_for_completed()
        robot.say_text(str(scj1) + " à " + str(scj2)).wait_for_completed()    
    
    # Fin de partie			
    if (scj1 >= 5):
        robot.say_text("Tu as gagné !").wait_for_completed()    
